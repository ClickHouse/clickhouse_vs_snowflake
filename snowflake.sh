...
if [[ -z "${SNOWFLAKE_CONNECTION}" ]]; then
  ...
  echo "using connection ${SNOWFLAKE_CONNECTION}"
  SNOWFLAKE_CONNECTION="--connection ${SNOWFLAKE_CONNECTION}"
fi

if [[ -z "${QUERY_FILE}" ]]; then
  QUERY_FILE="snowflake_queries.sql"
else
...
cat $folder/${QUERY_FILE} | while read query; do
    ...
    for i in $(seq 1 $TRIES); do
        RES=$(echo $query | snowsql ${SNOWFLAKE_CONNECTION} --schemaname ${SNOWFLAKE_SCHEMA:=PYPI} --dbname ${SNOWFLAKE_DB:=PYPI} --warehouse ${SNOWFLAKE_WAREHOUSE:=2XLARGE} -o echo=False -o timing=True -o friendly=False | grep -E -o 'Time Elapsed: [0-9.]+s' | grep -E -o '[0-9.]+' 2>&1)
        if [ "$?" == "0" ] && [ "${#RES}" -lt "10" ]; then
            echo "${QUERY_NUM}, ${i} - OK"
